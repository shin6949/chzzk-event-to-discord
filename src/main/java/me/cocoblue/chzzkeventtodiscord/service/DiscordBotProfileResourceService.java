...
public class DiscordBotProfileResourceService {
    ...
    public Page<DiscordBotProfileDataEntity> list(ChzzkPrincipal principal, Pageable pageable) {
        ...
        return discordBotProfileDataRepository.findAllByOwnerId_ChannelId(principal.channelId(), pageable);
    }
    ...
    public DiscordBotProfileDataEntity create(
        ...
        try {
            return discordBotProfileDataRepository.saveAndFlush(DiscordBotProfileDataEntity.builder()
                .ownerId(owner)
    ...
    public DiscordBotProfileDataEntity update(
        ...
        final String nextObjectKey = avatar == null || avatar.isEmpty()
            ? null
        ...
        try {
            final DiscordBotProfileDataEntity saved = discordBotProfileDataRepository.saveAndFlush(entity);
            if (nextObjectKey != null) {
    ...
    private DiscordBotProfileDataEntity findReadable(Long id, ChzzkPrincipal principal) {
        if (principal.role() == AppRole.ADMIN) {
            return discordBotProfileDataRepository.findById(id)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "bot profile not found"));
        }
        return discordBotProfileDataRepository.findByIdAndOwnerId_ChannelId(id, principal.channelId())
            .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "bot profile not found"));
    }
    ...
    private void ensureWritable(DiscordBotProfileDataEntity entity, ChzzkPrincipal principal) {
        ...
        if (!Objects.equals(entity.getOwnerId().getChannelId(), principal.channelId())) {
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, "bot profile does not belong to authenticated user");
        }
    }
